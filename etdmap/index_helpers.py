...
import pandas as pd
import logging
from etl.validators.dataset_validators import dataset_flag_conditions
...
def read_metadata(metadata_file: str, required_columns=["HuisId"]) -> pd.DataFrame:
    ...
    if all(col in df.columns for col in required_columns):
        ...
        logging.error(
            f'Not all required columns in sheet "Data" in metadata file: {metadata_file}'
        )
        raise Exception(
            f'Not all required columns in sheet "Data" in metadata file: {metadata_file}'
        )
...
bsv_metadata_df = read_metadata(
    bsv_metadata_file, required_columns=bsv_metadata_columns
)


def read_index() -> (pd.DataFrame, str):
    """
    Reads the index parquet file from the specified folder path.

    Returns:
    tuple: A tuple containing the DataFrame of the index and the path to the index file.
    """
    index_path = os.path.join(mapped_folder_path, "index.parquet")
    if os.path.exists(index_path):
        index_df = pd.read_parquet(index_path)
    else:
        index_df = pd.DataFrame(columns=["HuisId", "HuisCode", "Dataleverancier"])
...
def update_index(
    index_df: pd.DataFrame, new_entry: dict, data_provider: str
) -> pd.DataFrame:
    """
    Updates the index with new entries and recalculates or adds flag columns for dataset validators.
    ...
    if new_entry["HuisId"] in index_df["HuisId"].values:
        index_df.loc[
            index_df["HuisId"] == new_entry["HuisId"], ["HuisCode", "Dataleverancier"]
    ...
    dataset_file = os.path.join(
        mapped_folder_path, f"household_{household_code}_table.parquet"
    )
    if os.path.exists(dataset_file):
        df = pd.read_parquet(dataset_file)
        for flag, condition in dataset_flag_conditions.items():
            # Add the flag column if it does not exist and ensure it's of BooleanDtype
            if flag not in index_df.columns:
                index_df[flag] = pd.Series(pd.NA, dtype="boolean", index=index_df.index)
            try:
                ...
                logging.error(
                    f"Error validating with {flag} for household {household_code}: {e}",
                    exc_info=True,
                )
                index_df.loc[index_df["HuisCode"] == household_code, flag] = pd.NA
...
def update_meta_validators(index_df):
    ...
    if all(col in index_df.columns for col in cols):
        ...
        index_df["validate_cumulative_diff_ok"] = pd.Series(
            pd.NA, dtype="boolean", index=index_df.index
        )
...
def update_meenemen() -> pd.DataFrame:
    logging.info('Updating index with which households to include in column "Meenemen"')
...
def add_metadata_to_index(
    index_df: pd.DataFrame, metadata_df: pd.DataFrame, data_leverancier=None
) -> pd.DataFrame:
    """
    ...
    data_leverancier (str): The data supplier name, if not present in metadata_df.
    ...
    if data_leverancier is None:
        raise Exception(
            "Need to provide a supplier explicitly (data_leverancier is None)."
        )
    ...
    # Check that the combination values of shared columns is the same across the three datasets: index_df, metadata_df, and bsv_metadata_df and add a warning if not
    ...
    if not inconsistent_combinations.empty:
        logging.warning(
            "The following combinations of shared column values are inconsistent across the datasets:"
        )
    ...
    if not "Dataleverancier" in metadata_df.columns:
        if data_leverancier is None:
    ...
    # add Meenemen, Notities, and protected_columns using data from bsv_metadata_df (do not change bsv_metadata_df)
    ...
    index_df.set_index(["HuisId", "ProjectId", "Dataleverancier"], inplace=True)
    metadata_df.set_index(["HuisId", "ProjectId", "Dataleverancier"], inplace=True)
