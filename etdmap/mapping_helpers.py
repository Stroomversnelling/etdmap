...
def rearrange_model_columns(
    household_df: pd.DataFrame, add_columns: bool = True, context=""
) -> pd.DataFrame:
    if context != "":
        context = context + ": "
    for col, expected_type in model_column_type.items():
        if col in household_df.columns:
            ...
            if actual_type != expected_type:
                logging.warn(
                    f"{context}Column '{col}' has type '{actual_type}' but expected type is '{expected_type}'"
                )
    if add_columns:
        ...
        for col in model_column_order:
            if col not in household_df.columns:
                logging.warn(
                    f"{context}Missing column {col} added and filled with NA values."
                )
                household_df[col] = pd.Series(
                    pd.NA, dtype=model_column_type[col], index=household_df.index
                )
    else:
...
def validate_cumulative_variables(
    ...
    for col in cumulative_columns:
        if col in group.columns:
            # Check for gaps greater than the specified timedelta after first value
            ...
            if (filtered_group["ReadingDateDiff"] > timedelta).any():
                ...
                max_gap_start = filtered_group.loc[
                    filtered_group["ReadingDateDiff"].idxmax(), "ReadingDate"
                ]
                max_gap_start_unix = max_gap_start.timestamp()
                logging.warn(
                    f"{context}Group has a gap of {max_delta} > allowed ({timedelta}) starting at {max_gap_start} ({max_gap_start_unix}) in 'ReadingDate' for column '{col}'."
                )
            ...
            if any(filtered_group["negative_diff"]):
                ...
                logging.warn(
                    f"{context}Column {col} has a decrease in subsequent cumulative values at 'ReadingDate': {reading_dates.to_list()}."
                )
                ...
                if any(filtered_group["zero"]):
                    ...
                    logging.warn(
                        f"{context}Column {col} has unexpected zero values in cumulative values from {first_date} to {last_date}. This will be removed"
                    )
                    result["no_unexpected_zero"] = False

            if group[col].ffill().notna().sum() / len(group) < available:
                logging.warn(
                    f"{context}Column '{col}' in group has less than {available*100}% non-NA values."
                )
                result["enough_values"] = False

        else:
            logging.error(f"{context}Column '{col}' not found in the DataFrame.")
...
def add_diff_columns(
    ...
    def calculate_diff(group):
        ...
        if not all(valid_result.values()):
            invalid = [key for key, value in valid_result.items() if value == False]
            if drop_unvalidated == True:
                logging.error(
                    f"{context_string}Some cumulative columns did not pass validation ({invalid}). Dropping group/data."
                )
                return pd.DataFrame()  # Return empty DataFrame to drop invalid group
            else:
                logging.warning(
                    f"{context_string}Some cumulative columns did not pass validation ({invalid}). Keeping group/data."
                )
                ...
                if not valid_result["no_negative_diff"]:
                    ...
                    for rd in reading_dates:
                        ...
                        if not next_value_row.empty:
                            ...
                            if next_value >= -1 * gap:
                                logging.info(
                                    f"{context_string}Removing unexpected zeros from '{col}' between {rd} and {next_value_date}"
                                )
                                group.loc[
                                ...
                                logging.error(
                                    f"{context_string}Two negative diffs one after the other between {rd} and {next_value_date}. Will remove all these values for {col}."
                                )
                                group.loc[
                                    (group["ReadingDate"] >= rd)
                                    & (group["ReadingDate"] < next_value_date),
                                ...
                                if (
                                    group.loc[
                                        group["ReadingDate"] == rd, col + "Diff"
                                    ].iloc[0]
                                    < 0
                                ):
                                    logging.info(
                                        f"{context_string}Negative gap jump at {rd}. Removing single cumulative value."
                                    )
                                    group.loc[group["ReadingDate"] == rd, col] = pd.NA
                                    recalculate = True
                                else:
                                    logging.info(
                                        f"{context_string}Negative gap jump at {rd}. Diff is not negative, not removing any values."
                                    )
                        else:
                            recalculate = True
                            group.loc[(group["ReadingDate"] >= rd), col] = pd.NA
                            logging.error(
                                f"{context_string}Removing all values in '{col}' after date '{rd}' as there are no subsequent increases after the negative diff."
                            )
                    ...
                    if any(group[col + "Diff"] < 0):
                        logging.error(
                            f"{context_string}Removed zeros but diff still has negative values! Check data and consider removing."
                        )

            else:
                logging.warn(
                    f"{context_string}Cumulative column '{col}' not found. No Diff column created."
                )
    ...
    if isinstance(data, pd.core.groupby.DataFrameGroupBy):
        ...
        raise TypeError(
            "{context_string}Input data must be a pandas DataFrame or a pandas GroupBy object."
        )
...
def fill_down_infrequent_devices(
    ...
    # This function is potentially problematic if the data source or devices are misbehaving as the imputation will not be run on it.
...
def ensure_intervals(
    df: pd.DataFrame, date_column: str = "ReadingDate", freq="5min"
) -> pd.DataFrame:
    ...
    expected_num_records = int((latest - earliest) / pd.Timedelta(minutes=5)) + 1

    if expected_num_records == len(df.index):
        logging.info(
            f"Expected number of records based on start and end date. Not attempting to add {freq} intervals."
        )
        return df
    ...
    if expected_num_records > len(df.index):
        ...
        if len(merged_df.index) > expected_num_records:
            logging.error(
                f"There are more records than possible if {freq} interval would be respected. Merging left to reduce records.  Check data source."
            )
